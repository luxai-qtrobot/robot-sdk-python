...
def fg_audio_volume(robot: Robot):
    ...
    vol = robot.media.get_fg_audio_volume().result()
...
def bg_audio_volume(robot: Robot):
    ...
    vol = robot.media.get_bg_audio_volume().result()
...
def play_fg_audio_file(robot: Robot):
    ...
    h = robot.media.play_fg_audio_file(audio_file_on_robot, blocking=False)
    time.sleep(5)
    h.cancel()
...
def pause_resume_fg_audio_file(robot: Robot):
    ...
    play_handler = robot.media.play_fg_audio_file(audio_file_on_robot, blocking=False)
    time.sleep(10)
...
def play_fg_bg_audio_files(robot: Robot):
    ...
    h_bg = robot.media.play_bg_audio_file(bg_audio_file_on_robot, blocking=False)

    # start FG audio after a short delay to create overlap
    time.sleep(5)
    h_fg = robot.media.play_fg_audio_file(fg_audio_file_on_robot, blocking=False)
...
def play_fg_online_audio_file(robot: Robot):
    ...
    h = robot.media.play_fg_audio_file(online_audio_url, blocking=False)
    time.sleep(10)
    h.cancel()
...
def play_fg_online_radio(robot: Robot):
    ...
    h = robot.media.play_fg_audio_file(online_radio_url, blocking=False)
